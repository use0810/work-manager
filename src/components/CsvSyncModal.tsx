  ...
  async function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (result.records.length === 0) {
        setBanner({
          kind: 'error',
          text: `${result.totalRows} 行ありましたが、形式が合わずすべて読み取れませんでした。ヘッダ「id,startAt,endAt,category,categoryOption,categories,memo」を確認してください。`,
        });
        return;
      }
      setImported(result);
          ...
          {mode === 'menu' && (
            <>
              ...
              <section className="csv-section">
                ...
                <p className="csv-section-desc">
                  以前書き出した CSV、または同じ列構成（id, startAt, endAt, category, categoryOption, categories, memo）の CSV を読み込めます。旧形式も取り込み可能です。
                </p>
